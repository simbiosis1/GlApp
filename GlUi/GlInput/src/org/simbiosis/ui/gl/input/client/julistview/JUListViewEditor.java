...
import org.simbiosis.ui.gl.input.client.editor.GLTransListViewTable;
...
public class JUListViewEditor extends Composite implements Editor<FindTransDv> {

	private static JUListEditorUiBinder uiBinder = GWT
			.create(JUListEditorUiBinder.class);

	interface JUListEditorUiBinder extends UiBinder<Widget, JUListViewEditor> {
	}

	interface Driver extends
			SimpleBeanEditorDriver<FindTransDv, JUListViewEditor> {
	}

	private Driver editorDriver = GWT.create(Driver.class);

	@UiField
	GLTransListViewTable resultTable;

	public JUListViewEditor() {
